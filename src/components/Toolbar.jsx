...
const DATA_FORMULAS = [
  ...
  { label: 'FIND_AND_REPLACE', example: '=FIND_AND_REPLACE(:,"find","replace")' }
];
...
const Toolbar = ({ 
  ...
  return (
    <div className="toolbar bg-white p-1 flex items-center gap-1 border-b border-gray-300">
      ...
      <button 
        className={`toolbar-btn ${cellStyles?.fontWeight === 'bold' ? 'bg-gray-200' : ''}`}
        onClick={onBold}
        title="Bold (Ctrl+B)"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
      ...
      <button 
        className={`toolbar-btn ${cellStyles?.fontStyle === 'italic' ? 'bg-gray-200' : ''}`}
        onClick={onItalic}
        title="Italic (Ctrl+I)"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
      ...
      <div className="flex items-center align-btns">
        <button 
          className={`toolbar-btn ${cellStyles?.textAlign === 'left' ? 'bg-gray-200' : ''}`}
        ...
        <button 
          className={`toolbar-btn ${cellStyles?.textAlign === 'center' ? 'bg-gray-200' : ''}`}
        ...
        <button 
          className={`toolbar-btn ${cellStyles?.textAlign === 'right' ? 'bg-gray-200' : ''}`}
